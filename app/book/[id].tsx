...
import { AlertCircle, AlertTriangle, CheckCircle, Clock, FileText, XCircle } from "lucide-react-native";
...
export default function BookDetailsScreen() {
    ...
    if (error || !data?.job) {
        return (
            <Box className="flex-1 bg-background-0 justify-center items-center p-6">
                ...
                <Text className="text-typography-900 font-bold text-lg mb-2">Failed to load details</Text>
                <Text className="text-typography-500 text-center">We couldn't fetch the book information. Please try again later.</Text>
            </Box>
        );
    }
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            default: return "text-typography-500";
        }
    };

    const getStatusIcon = (status: string) => {
        switch (status) {
            case "COMPLETED": return <CheckCircle size={24} className="text-success-500" />;
            case "FAILED": return <XCircle size={24} className="text-error-500" />;
            case "PROCESSING": return <Clock size={24} className="text-primary-500" />;
            default: return <Clock size={24} className="text-typography-500" />;
        }
    };

    return (
        <Box className="flex-1 bg-background-0">
            <Stack.Screen options={{ title: "Book Details", headerBackTitle: "Back" }} />
            <SafeAreaView className="flex-1" edges={['bottom', 'top']}>
                <ScrollView className="flex-1" contentContainerStyle={{ paddingVertical: 50, paddingHorizontal: 24 }}>
                    <VStack space="4xl">
                        {/* Header Info */}
                        <Center className="mb-4">
                            ...
                            <Heading size="xl" className="text-center text-typography-900 font-bold mb-1">
                                {job.originalFilename}
                            </Heading>
                            <HStack space="xs" className="items-center bg-background-50 px-3 py-1 rounded-full border border-outline-100">
                                {getStatusIcon(job.status)}
                                <Text className={`font-bold ${getStatusColor(job.status)} uppercase text-xs tracking-wider`}>
                                    {job.status}
                                </Text>
                            </HStack>
                        </Center>
                        ...
                        <HStack space="md" className="justify-between">
                            <Box className="flex-1 bg-background-50 p-4 rounded-xl border border-outline-100 items-center">
                                <Text className="text-typography-400 text-xs uppercase font-bold mb-1">Characters</Text>
                                <Text className="text-typography-900 font-bold text-lg">
                                    {(job.totalCharacters || 0).toLocaleString()}
                                </Text>
                            </Box>
                            <Box className="flex-1 bg-background-50 p-4 rounded-xl border border-outline-100 items-center">
                                <Text className="text-typography-400 text-xs uppercase font-bold mb-1">Created</Text>
                                <Text className="text-typography-900 font-bold text-lg">
                            ...
                            <VStack className="pl-4 border-l-2 border-outline-100 ml-2 space-y-8 pb-4">
                                {sortedEvents.map((event, index) => (
                                    <Box key={event.id} className="relative pl-6">
                                        {/* Timeline Dot */}
                                        <Box className={`absolute -left-[21px] top-1 w-3 h-3 rounded-full border-2 border-background-0 ${event.status === 'FAILED' ? 'bg-error-500' : 'bg-primary-500'}`} />
                                ...
                                {sortedEvents.length === 0 && (
                                    <Text className="text-typography-400 text-sm italic pl-6">No activity recorded yet.</Text>
                                )}
                            </VStack>
                        </VStack>
                    </VStack>
